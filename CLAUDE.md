...
- **Paid ads:** Customer-only Instagram/Facebook ads are drafted in weekly batches in the admin panel (Marketing → Ads) by `php artisan ads:run`. Dayne gets each batch by email 48h before go-live, then launches approved batches in Ads Manager by hand (no Meta API yet). See `docs/paid-ads.md`.
